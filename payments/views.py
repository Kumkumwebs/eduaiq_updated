...
def create_razorpay_order_api(request):
    """API endpoint to create a Razorpay Order ID"""
...
@csrf_exempt
def verify_razorpay_signature_api(request):
    """API endpoint to verify Razorpay signature and log payment transaction"""
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Only POST allowed'}, status=405)
    try:
        data = json.loads(request.body) if request.body else request.POST
        order_id = data.get('razorpay_order_id', '')
        payment_id = data.get('razorpay_payment_id') or f"pay_{uuid.uuid4().hex[:14]}"
        signature = data.get('razorpay_signature', '')
        amount = float(data.get('amount', 0))
        description = data.get('description', 'Razorpay Online Payment')
        source_type = data.get('source_type', 'miscellaneous_fee')
        
        txn = Transaction.objects.create(
            invoice_no=f"INV-{uuid.uuid4().hex[:6].upper()}",
            transaction_type=description[:50],
            payment_type='Razorpay Online',
            amount=amount,
            payer=request.user if (hasattr(request, 'user') and request.user.is_authenticated) else None,
